...
- **输入管线**:名称 / SMILES / 分子式 / 蛋白质序列四种输入——中文名回译校验防翻译错、分子式自动枚举同分异构体(小键盘输入,下标显示)、氨基酸序列经 ESMFold 在线预测 3D 结构。
- **交互建模**:9 种预设基团拖拽成键、一键补氢、四模式拖动(gizmo/锁轴)、拖拽连接提示、单键二面角旋转、双键顺反翻转、选中原子旁删除图标。
- **可视化**:青色水滴形孤对电子云(NEBOOK 风格)、四种原子材质风格(自发光/经典/金属/卡通)、网格吸附、拖拽 HUD、悬停高亮。
- **自测体系**:页面内自动测试 + Playwright 回归脚本(33 几何 + 23 π + 33 + 23 + 36 基团方位)。
...
- **名称 / SMILES / 分子式 / 序列输入**(右上角 🧪 输入面板):四种输入方式——
  - *名称*:输入英文名(ethanol)或中文名(乙醇/阿司匹林),内置库 + PubChem 在线解析;中文名先经回译校验(翻译后反译比对,防误译如 环辛四烯→环十八碳烯),命中修正表时直连。
  - *SMILES*:输入结构式(如 CCO / c1ccccc1),优先取 PubChem 3D 坐标(含 H、几何好),失败回退本地管线。
  - *分子式*:输入化学式(如 C6H6O / O2),自动弹出**同分异构体选择面板**(在线枚举,排除氘代/同位素/组合物),可单选或多选并排生成;**小键盘**输入元素+数字(下角标显示如 C₆H₆),点击别处自动收起。
  - *蛋白质序列*:输入氨基酸序列(中文名/三字母/单字母混合均可,≤400 残基),经 **ESMFold** 在线预测 3D 结构(必须联网)。
- **自由搭骨架**:侧面原子卡片拖出 **8 种原子**(C/H/O/N/F/Cl/Br/I)到画布,靠近已有原子松开即自动成键。
- **基团拖拽**:侧栏 **9 个预设基团** —— 甲基 CH₃ / 乙基 C₂H₅ / 苯基 Ph / 氨基 NH₂ / 羧基 COOH / 羰基 C=O / 醛基 CHO / 酯基 COOR / 羟基 OH,拖到分子上自动吸附到最近可键合原子,绕连接轴 6 角度选无碰撞朝向(优先替换目标上的 H)。
- **一键补氢**(`btn-toggle-hydro`):开启后每次「全局同步优化」自动按价态补齐缺失 H(如乙烷+8、乙烯+4、乙炔+2),再经优化器校正到理想角。
- **删除**:点「删除选中」、按 Delete 键,或点选中原子旁出现的红色 ✕ 图标;「清空」重置画布。
...
- **离域π键**(`btn-toggle-deloc`):开启时杂化推断考虑共轭(苯酚/苯胺 lp 参与共轭→sp2),并可「显示离域π键」(`btn-toggle-pi`)高亮 π 体系。「离域π键自测」一键跑检测并报告。
- **孤对电子**(`btn-toggle-lp`):每对孤对显示为青色水滴云 + 双黑点(NEBOOK 风格),方向随 VSEPR 模板实时更新(如水中 O 两对、氨中 N 一对)。
- **原子材质**(设置面板「原子材质」下拉):自发光(霓虹,忽略场景光)/ 经典光泽 / 金属 / 卡通 四种风格,实时切换并记忆。
...
| `球棍模型3.html` | 离域π键专题副本:芳香/共轭体系检测、π 键高亮、`runPiSystemTests()` 23 项自测 |
| `球棍模型4.html` | 功能最全版:输入管线(名称/SMILES/分子式异构体/蛋白质 ESMFold)+ 交互建模全功能集(见下) |
...
- 全部自测当前状态:**33 几何 + 23 π + 33 + 23 + 36 基团方位全绿**。
...
- `window.__dragMode`{project/raycastGizmo/atoms/...}、`window.__torsion`、`window.__cisTrans`、`window.__conjBridge`{is/list/tryPivot}、`window.__attachDeleteIcon`/`__detachDeleteIcon`
- `window.__lpStats`(孤对统计)、`window.__connectHint`、`window.__piSystemsSummary`、`window.__atomStyle`{get/set/list}
- 输入管线:`window.__parseFormula`/`__toSubscriptFormula`/`__fetchFormulaIsomers`/`__showIsomerPanel`/`__generateSelectedIsomers`/`__parseAaSequence`/`__fetchProteinPDB`/`__generateFromProteinSeq`/`__translateZhToEn`
