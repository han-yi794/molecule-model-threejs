// debug_styrene_final.js — UI 路径苯乙烯共面测量
...
(async () => {
    ...
    page.on('console', m => { const t = m.text(); if (t.includes('[分子来源]')) console.log('LOG:', t); if (m.type() === 'error') errs.push(m.text().slice(0, 200)); });
    await page.goto('http://127.0.0.1:' + PORT + '/%E7%90%83%E6%A3%8D%E6%A8%A1%E5%9E%8B4.html?noauto=1&v=stf', { waitUntil: 'networkidle', timeout: 60000 });
    ...
    const out = await page.evaluate(async () => {
        ...
        return {
            ...
            outers,
            nonHzDistFromPlane: pos.filter(p => p.type !== 'H').map(p => ({ id: p.id, t: p.type, d: +dist(p.id).toFixed(3) })).sort((a, b) => Math.abs(b.d) - Math.abs(a.d)).slice(0, 8),
            bondTypes: bonds.map(b => b.type).join(',')
        };
    });
